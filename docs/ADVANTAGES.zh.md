...
> 面向技术决策者 · 基于 2026-08 的实际部署（dev-17，新加坡）与实测数据
...
- **会话粘性（session affinity）**：同一会话稳定落到同一后端，prompt 缓存命中率不被打散
...
（[CAPACITY.zh.md](CAPACITY.zh.md)，dev-15 / dev-16 / dev-17 三个环境结论一致）。
...
- **实测零丢失**：dev-17 压测 1,808 个请求，终局对账无缺失
...
| **App Insights** | 遥测（会采样） | 调用次数、p50/p95、**网关 vs 后端延迟拆分**、失败数、每小时趋势 |
...
dev-17 实际账单，3 个算力账号，Southeast Asia 按需价：
