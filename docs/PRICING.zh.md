...
> 所有用量取自 `az` 对**正在运行的 dev-17** 的读数(SKU、副本数、vCPU/内存、存储大小)。
...
² **`cosmos_throughput_rus` 的默认值已改为 400(预配)**,所以**下一个新建的环境**
会直接建成预配模式。预配折合 $0.0222/百万 RU,serverless 是 $0.285/百万 RU,
**差 12.8 倍**,盈亏平衡在**利用率约 8%**(400 RU/s 下约等于 800 万次调用/月)。

> ⚠️ **400 RU/s 只够稳态写入,扛不住压测或大盘密集查询。** dev-18(2026-08-09)实测:
> 冒烟期间 RU 利用率 8%,压测一开始就 **100% 打满并持续 45 分钟以上**,6 小时内
> **2,236 次 429**(占 Cosmos 请求 23%)。
>
> 上面那个盈亏平衡只算了**写入**(每次调用约 10 RU),漏了三样:导入器的**批量突发**
> 写入(Capture 每 300 秒吐一批,不是匀速)、**跨分区聚合查询**(单次最坏 5,000 RU,
> **一次吃掉 400 RU/s 预算的 12.5 秒**)、以及对账/大盘轮询。serverless 按用量付费、
> 无容量上限,所以这个问题在前四轮战役里从未暴露。
>
> **没有因此丢数据**(SDK 对 429 自动重试,终局文档数精确等于预测),但导入延迟被
> 显著拉长。要跑压测或有密集大盘使用,应当上调到 1000+ RU/s 或改用自动缩放
> (`autoscale max 1000`,闲时按 10% 计费);纯稳态低负载才适合 400。
> 详见 [CAPACITY.zh.md](CAPACITY.zh.md) §7.6。
...
3. **Log Analytics 采样 / 每日上限** —— 现在 `sampling_percentage = 100`(每次调用
   ...
   **3,940 B → 705 B,降 82% 而非 90%** —— customMetrics 那 346 B 是地板。
   ...
   > ⚠️ **采样会破坏精确对账。** 闭合判据 `网关200 == Cosmos带token + Σhub lost`
   > 用的就是 App Insights 的 `requests` 计数,只记 10% 就得靠 ÷0.1 外推,从"逐条
   > 相等"退化成"统计估计"。customMetrics 和 Cosmos 不受影响,丢的是**网关侧请求
   > 计数的精度**。折中做法:平时 10%,做对账战役时临时调回 100%(APIM diagnostic
   > 的属性改动秒级生效,不用重建)。

4. **Cosmos 预配已是新环境的默认**(`cosmos_throughput_rus = 400`)。已存在的
   serverless 环境切不动,也不该切 —— 见上方注 ²。
...
2026-08 取数。用量:`az` 对 dev-17 运行态的读数。换区域 / 协议价请用
[Azure 定价计算器](https://azure.microsoft.com/zh-cn/pricing/calculator/)复核。
